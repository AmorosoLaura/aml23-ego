...
class EpicKitchensDataset(data.Dataset, ABC):
    ...
    def _get_train_indices(self, record, modality='RGB'):
        ...
        if self.dense_sampling[modality]:
            ...
            total_clips=self.num_frames_per_clip[modality]*self.num_clips
            average_duration = record.num_frames[modality] // (total_clips)
            
            if average_duration <= 0:
                indices = np.zeros((self.num_frames_per_clip[modality] * self.num_clips,))
            else:
               
                range_numbers=np.arange(total_clips)
               
                intervals_idx=np.multiply(range_numbers, average_duration)
                #for each interval select a random frame index
                random_indices=np.random.randint(average_duration, size=total_clips)
