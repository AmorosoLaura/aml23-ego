...
import torch.optim
import torch
from utils.loader_midlevel import MidLevelLDataset
...
def main():
    ...
    num_classes, valid_labels, source_domain, target_domain = utils.utils.get_domains_and_labels_action_net(args)
    ...
    for m in modalities:
        ...
        models[m] = getattr(model_list, args.models[m].model)(num_classes=args.dataset.num_classes, hidden_size=args.models.hidden_size)
    ...
    if args.action == "train":
        ...
        train_loader = torch.utils.data.DataLoader(MidLevelLDataset(args.dataset.shift.split("-")[0], modalities,
                                                                       'train', args.dataset,
...
def train(action_classifier, train_loader, val_loader, device, num_classes):
    ...
    for i in range(iteration, training_iterations):
        ...
        for m in modalities:
            data[m] = source_data[m]
            for i in range(2):
                data[m][i] = source_data[m][i].to(device)
...
def validate(model, val_loader, device, it, num_classes):
    ...
    with torch.no_grad():
        ...
        class_accuracies = [(x / y) * 100 if y > 0 else 0.0 for x, y in
                            zip(model.accuracy.correct, model.accuracy.total)]
